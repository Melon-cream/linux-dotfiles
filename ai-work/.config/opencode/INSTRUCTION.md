...
- どうしても解決できない複雑な問題がある場合は `/rescue` でレスキューエージェントを呼び出す。
...
- 単発で終わらない作業だと判断される場合は "planning-with-files" スキルを使う。
...
委譲する場合はまず skill opencode-delegate-herdr を読み、その手順 (herdr pane split / opencode run / OPENCODE_EXIT 待ち) に従うこと。
